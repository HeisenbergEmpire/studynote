...
# 以下用加拿大稅法為例子，意思為：
# 當商品價格大於等於1元，則稅收為7%，顯示稅率；
# 當商品價格不在大於等於1的範圍內，則稅收為0，顯示稅率；
...
# 除了上述的方式，亦有下面縮略的表現方法，意思為：
# 如果商品價格大於等於1，則稅率為7%；
# 如果商品價格不在上述範圍內的，則稅率為0。打印稅率
# 這種方法比較簡潔，但首先一定要讀懂語法意義
if price >= 1.00:
    tax =  .07
else:
    tax = 0
...
price1 = float(price1)
# 記得加入float令數值從文字類型轉變為數字類型
if price1 >= 1.00:
    tax = .07
else:
    tax = 0
print('Tax rate is: ' + str(tax))
# 要記得輸入str將tax從數字類型轉變為文字類型
print()
# 除上述亦可以這樣簡化出來
