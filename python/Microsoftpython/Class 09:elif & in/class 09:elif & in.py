# 如果有多個篩選項目，只用if-else就太重重複複，這時就可以用elif（否則如果），還是要記得後面要有冒號
...
# elif後面亦可以繼續加用else
print()
# 上面由於Alberta和Nunavut省的稅率是一樣的，所以我們可以縮略寫成這樣
...
if sheng == 'Alberta' or sheng == 'Nunavut':
    ...
    tax = 0.15
print('Tax rate is: ' + str(tax))
# 當有多個or選項時，我們則可以直接用in來縮略，注意只能用於多項條件為同一結果的前提下（上例只有兩個條件也可用in，看起來更簡潔）
...
if sheng1 in ('Alberta','Nunavut','Yukon'): 
    ...
    tax = 0.15
print('Tax rate is: ' + str(tax))
# 其實不論斜槓還是冒號，都可以縮在一行，但為了代碼規範清晰明確最好不要
print()
# if和其他一樣，可以多重嵌套，雖注意這樣要滿足第一個條件才可繼續第二個條件
...
print('Tax rate is: ' + str(tax))
# 按照邏輯順序，將省的問題插到了嵌套中間
# 加上了capitalize就不用擔心用戶輸入大小寫的關係引致if檢驗錯誤的問題
